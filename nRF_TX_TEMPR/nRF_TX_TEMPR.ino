...
const int tempSensor = A0;    //LM35 connected to Analog 0
float tempr;                  //Variable to store sensor
...
void loop()
{
  ...
  {
    tempr = (500.0 * analogRead(tempSensor))/1024;
    Serial.print("Temperature to be sent: ");
    Serial.println(tempr);        //Print the packet number being sent
    ...
    if(!uno.write(&tempr, 4))       //If the send fails, let user know through serial monitor
    {
